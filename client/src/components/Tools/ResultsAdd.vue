<template>
	<div class="whitelist container col-md-12">
		...
		<input class="datainput" type="text" v-model="resultsData" placeholder="Enter results">
...
export default {
	...
	methods: {
		submit() {
			this.$http.post(`/setElectionResults/${this.$route.params.id}`, { admin: this.admin, results: this.resultsData } ).then(result => {
				this.result = 'Results submitted';
			}).catch(error => {
				console.error(error);
