...
module EllipticAlgebra (Point (..), Curve,getGenerators, pointMul, pointAdd, pointInvert, getOrderOfGenerator,isPointOnCurve) where
import Data.List
import Data.Bits
...
-- Point Invert
pointInvert :: Curve -> Point -> Point
pointInvert _ Identity = Identity
pointInvert (_,_,p) (Coord x y) = Coord x ((-y) `mod` p)
...
getOrderOfGenerator :: Curve -> Point -> Integer
getOrderOfGenerator c g = 1 + foldr (\x -> (+) 1) 0 (takeWhile (/= Identity) $ map (pointMul' c g) [1..])
...
getGenerators c = filter (isPrime . getOrderOfGenerator c) allPoints
    where 
